# -*- coding: utf-8 -*-
"""
Created on Wed Aug 24 14:06:20 2022

@author: Judy
"""


modelName="AlexNet_IBK3E50_BLURk5_sobel3_o2s8_shz_india" #224
...
from extraFunction.ImageProcessing_256 import cut_letterboxing,squImg,avgBlur,BLUE_EDGE,resizeImg,ORI_EDGE
from extraFunction.WriteMataData import write_csv,addData,CSVAdd_ArrImage
from extraFunction.AlexNet import alexNet
...
if __name__=="__main__":
    ...
    DataBase=os.path.join(basePath,"SHZ_DataBase")
    make_file(DataBase)
    ...
    make_file(save_trainingProcessImg)
    print("顯示原始圖片路徑:")
    print(szData_normal,szData_TB,IndiaData_normal,IndiaData_TB)
    ...
    IndiaValid=os.path.join(validData,"IndiaValid")
    make_file(IndiaValid)
    
    #divide data train 80% / test 20% (換模型跑時需註解)
    # divide_data(szData_normal,shzTrain,shzTest)
    # divide_data(szData_TB,shzTrain,shzTest)
    
    # valid_data(IndiaData_normal,IndiaValid)
    # valid_data(IndiaData_TB,IndiaValid)
    ...
    #=========================== 10種模糊挑一種使用 ===========================
    avgBlur_k3=os.path.join(trainData,'ShzAvgBlur_k3')
    make_file(avgBlur_k3)    
    # aveBlur(GanImage,(3,3),avgBlur_k3) 
    
    #===================================CSV mataData檔(檔名;標籤;原始圖片大小)===================================
    #train
    # write_csv(shzTrain,"GanTrainMataData_BK3",trainData)
    # TrainData=pd.read_csv(os.path.join(trainData,"GanTrainMataData_BK3.csv"),encoding= 'utf-8')
    # addData(avgBlur_k3,TrainData,trainData,"GanTrainMataData_BK3")
    TrainData=pd.read_csv(os.path.join(trainData,"GanTrainMataData_BK3.csv"),encoding= 'utf-8')
    ...
    #valid
    # write_csv(IndiaValid, "ValidMataData", validData)
    ...
    CL_trainGanImage=os.path.join(trainData,"GanImage_BK3") #不動
    ...
    # cut_letterboxing(avgBlur_k3, CL_trainGanImage) 
    
    # cut_letterboxing(shzTest, CL_allTestImage)    

    
    # ValidOriList=[IndiaData_normal,IndiaData_TB]
    ...
    squ_GanTrainImage=os.path.join(trainData,"squ_GanTrainImage_BK3")
    make_file(squ_GanTrainImage)    
    squ_allTestImage=os.path.join(testData,"squ_allTestImage")
    make_file(squ_allTestImage)    
    squ_allValidImage=os.path.join(validData,"squ_allValidImage_india")
    ...
    make_file(BlurTest)
    BlurValid=os.path.join(validData,"BlurValid_BK5_india")
    make_file(BlurValid)
    ...
    make_file(edgeTest)
    edgeValid=os.path.join(validData,"edgeValid_Sb3_india")
    make_file(edgeValid)
    ...
    #===================================#開始將圖片前處理 3.轉成相同大小===================================
    edit_train=os.path.join(trainData,"edit_train_256")
    make_file(edit_train)    
    edit_test=os.path.join(testData,"edit_test_256")
    make_file(edit_test)    
    edit_valid=os.path.join(validData,"edit_valid_india_256")
    make_file(edit_valid)
    
    #原圖也需resize
    oriImage_train=os.path.join(trainData,"oriImage_train_256")   
    make_file(oriImage_train)  
    oriImage_test=os.path.join(testData,"oriImage_test_256")   
    make_file(oriImage_test)  
    oriImage_valid=os.path.join(validData,"oriImage_valid_india_256")   
...
# 
    ...
    #===================================疊圖===================================
    ResAddweTrain=os.path.join(trainData,"ResAddweTrain_sobel_o2s8_256")
    make_file(ResAddweTrain)
    ResAddweTest=os.path.join(testData,"ResAddweTest_sobel_o2s8_256")
    make_file(ResAddweTest)
    ResAddweValid=os.path.join(validData,"ResAddweValid_sobel_o2s8_india_256")
    ...
    CSVAdd_ArrImage(ResAddweTrain,TrainData,trainData,"TrainArr_So2s8_256")
    CSVAdd_ArrImage(ResAddweTest,TestData,testData,"TestArr_So2s8_256")
    CSVAdd_ArrImage(ResAddweValid,ValidData,validData,"ValidArr_So2s8_india_256")
    ...
    nimg_train=img_train.reshape((1590, 256, 256, 3)).astype('float32')/255
    nimg_test=img_test.reshape((132, 256, 256, 3)).astype('float32') /255
    ...
    nimg_valid=img_valid.reshape((155, 256, 256, 3)).astype('float32') /255
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2,patience=3, min_lr=0.001,verbose=2)
